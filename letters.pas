...
program balses;
Const chk = 12;
//const letters : array[1..24] of string = ('A','a','Ą','ą','E','e','Ę','ę','Ė','ė','I','i','Į','į','Y','y','O','o','U','u','Ų','ų','Ū','ū');     //if lithuanina text
const letters : array[1..12] of char = ('A','a','E','e','I','i','Y','y','O','o','U','u');              // if non-lithuanian text
...
begin
    ...
    while (Current <> nil) and (lowercase(i) > lowercase(Current^.z) ) do
        begin
            Previous:=Current;
...
begin
    while s <> nil do
       begin
           writeln(output,' ', s^.z);     // space - so the file looks nicier
           s := s^.next;
       end;
    Close(output);
end;
        ...
        while (chr <> ' ') do
             begin
                  ...
                  read(input, chr);
                      ...
                      if ((ord(lowercase(chr)) < 123)      // z - 122
                       and(ord(lowercase(chr)) > 96))      // a - 97
                        or(ord(chr) = 39)                  // '
                        or(ord(chr) = 45)                  // -

                            then dobreak:=FALSE;

                  if dobreak and not eof(input) then break;
